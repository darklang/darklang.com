...
const Header: React.FC = () => {
  const actions: ActionIcon[] = [
    { name: 'learn', icon: BookIcon, angle: 170, description: 'Learn about packages' },
    { name: 'search', icon: PackageSearchIcon, angle: 145, description: 'Search for packages' },
    { name: 'create', icon: PlusIcon, angle: 110, description: 'Create new packages' },
    { name: 'update', icon: RefreshIcon, angle: 70, description: 'Update existing packages' },
    { name: 'publish', icon: UploadIcon, angle: 35, description: 'Publish your packages' },
    { name: 'share', icon: ShareIcon, angle: 10, description: 'Share packages with others' },
  ];
  ...
  return (
    <div className="relative pt-16">
      ...
      <div className="relative w-80 h-48 sm:w-96 sm:h-56 md:w-[500px] md:h-68 lg:w-[650px] lg:h-80 xl:w-[680px] xl:h-88 mx-auto">
        <svg className="hidden md:block absolute bottom-2 left-0 w-full h-full pointer-events-none" viewBox="0 0 100 50" preserveAspectRatio="none">
          <path
              ...
              <div className="flex items-center justify-center">
                <div className="relative flex items-center">
                  <div className="relative">
                    <IconComponent className={`w-12 h-12 text-gray-light group-hover:text-white-custom transition-all duration-300 ${isVisible ? 'transform-none' : 'scale-110'
                      }`} />
                  </div>

                  {/* Text label on the left for left-side icons */}
                  {isLeftSide && (
                    <span className={`text-xl text-[#676767] font-medium mr-3 whitespace-nowrap transition-all duration-500 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-2'
                      }`}>
                      {action.name}
                    </span>
                  )}
                  ...
                  {!isLeftSide && (
                    <span className={`text-xl text-[#676767] font-medium ml-3 whitespace-nowrap transition-all duration-500 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'
                      }`}>
                      {action.name}
                    </span>
                  )}
