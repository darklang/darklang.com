...
const ACTS = [
  ...
  },
  {
    name: "Review",
    caption:
      "Read what the agent wrote: the definitions it added, the callers that followed, and the access each one needs, including the one you refused.",
  },
  {
    name: "Ship",
    caption:
...
const SCRIPT: Ev[] = [
  ...
  ev({
    ...
    k: "cmd",
    typed: true,
    segs: [["dark "], ["diff", "kw"], [" agent/stripe"]],
    pause: 300,
  }),
  ev({
    act: 6,
    k: "out",
    pad: true,
    segs: [["5 new, 1 followed, "], ["1 still denied", "err"]],
    pause: 600,
  }),
  ev({ act: 6, k: "sel", name: "Api.checkout", pause: 1800 }),
  ev({ act: 6, k: "sel", name: "Payments.refund", pause: 2200 }),
  ev({
    act: 7,
    k: "cmd",
    typed: true,
  ...
  ev({ act: 7, k: "cmd", typed: true, segs: [["dark "], ["sync", "kw"]] }),
  ev({
    act: 7,
    k: "out",
    tone: "ok",
  ...
  ev({ act: 7, k: "out", browser: true, segs: [[""]], pause: 5200 }),
];
                      ...
                      <div className="min-h-0 overflow-hidden border-b border-[#2a2a2e] px-4 py-3 font-code text-[12.5px] leading-[1.75] md:border-r md:border-b-0 md:text-[13px]">
                        ...
                        {defs.map(d => {
                          ...
                          return (
                            <div
                              key={d.name}
                              className={`-mx-2 flex gap-2 rounded border-l-2 px-2 transition-all duration-300 ${
                                lit ? "opacity-100" : "opacity-30"
                              } ${
                                on
                                  ? "border-purple-lbg bg-purple-lbg/15"
                                  : "border-transparent"
                              }`}
                            >
                              <span
                                className={`w-2 shrink-0 transition-opacity duration-300 ${
                                  on
                                    ? "text-purple-lbg opacity-100"
                                    : "opacity-0"
                                }`}
                                aria-hidden="true"
                              >
