...
class DarkPackagesApi {
  ...
  private readonly typeUrl = 'http://dark-packages.dlio.localhost:11001/type/get';
  ...
  private buildSearchUrl(moduleName: string, entityTypes: string, searchDepth = 'direct'): string {
  ...
  private async fetchTypeDefinition(typeId: string): Promise<TypeDefinition | null> {
    try {
  ...
  private async getTypeString(typeObj: any): Promise<string> {
    ...
    if (Array.isArray(typeObj)) {
      if (typeObj.length === 2 && typeof typeObj[0] === 'string' && this.isTypeId(typeObj[0])) {
      ...
      return typeObj.length > 0 ? await this.getTypeString(typeObj[0]) : 'Unknown';
    }
  ...
  private async processParameters(parameters: any[] = []): Promise<{ name: string; type: string }[]> {
  ...
  async searchModuleData(moduleName: string): Promise<ApiResponse> {
    ...
    const url = this.buildSearchUrl(moduleName, 'module,function,type,constant');
  ...
  async getSubmodulesCount(moduleName: string): Promise<number> {
    try {
      ...
      return submodules.filter(subPath =>
        Array.isArray(subPath) && subPath.length === parts.length + 1
      ).length;
    } catch (error) {
      console.error(`Error fetching submodules count for ${moduleName}:`, error);
      return 0;
    }
  }

  async processApiResponseToPackageData(moduleName: string, apiData: ApiResponse): Promise<PackageData> {
    const parts = moduleName.split('.');
    const isRootModule = parts.length <= 2;
    const displayName = parts[parts.length - 1] || moduleName;

    if (isRootModule) {
      const submodules = (apiData.submodules || []).filter((subPath: string[]) =>
        Array.isArray(subPath) && subPath.length === parts.length + 1
      );
      ...
      const processedSubModules = await Promise.all(
        submodules.map(async (subPath: string[]) => {
          ...
          try {
            ...
            console.error(`Error fetching submodules count for module card ${fullName}:`, err);
          }
    ...
    const typeList = (apiData.types || []).map((type: ApiType) => ({
      ...
      description: type.description || `Type definition for ${type.name?.name || 'Unknown'}`
    }));

    const constantList = (apiData.constants || []).map((constant: ApiConstant) => ({
      name: constant.name?.name || 'Unknown',
      description: constant.description || `Constant definition for ${constant.name?.name || 'Unknown'}`
    }));
  ...
  async processModuleData(moduleName: string, apiData: ApiResponse): Promise<ProcessedItemData> {
    const parts = moduleName.split('.');
    ...
    const fullConstantData = (apiData.constants || []).map((constant: ApiConstant) => ({
      name: constant.name?.name || 'Unknown',
      description: constant.description || 'No description available'
    }));

    return {
      functions: (apiData.fns || []).map((fn: ApiFunction) => fn.name?.name || 'Unknown'),
      types: (apiData.types || []).map((type: ApiType) => type.name?.name || 'Unknown'),
      constants: (apiData.constants || []).map((constant: ApiConstant) => constant.name?.name || 'Unknown'),
      submodules: processedSubmodules,
      fullFunctionData,
      fullTypeData,
      fullConstantData
  ...
  async fetchSiblingModules(parentModule: string): Promise<Array<{ name: string; path: string; stats: ModuleCounts & { submodules: number } }>> {
    ...
    return Promise.all(
      processedData.submodules.map(async (submodule) => {
        ...
        try {
          ...
          console.error(`Error fetching submodules count for ${submodule.fullName}:`, err);
        }
  ...
  async processInitialSidebarPackages(subModules: SubModule[]): Promise<Array<{ name: string; path: string; stats: ModuleCounts & { submodules: number } }>> {
    return Promise.all(
      subModules.map(async (sub: SubModule) => {
        ...
        try {
          ...
          console.error(`Error fetching initial submodules count for ${sub.fullName}:`, err);
        }
  ...
  async searchInModules(moduleNames: string[], searchTerm?: string): Promise<Map<string, ProcessedItemData>> {
      ...
      try {
        ...
        if (searchTerm) {
          ...
          const filteredData: ProcessedItemData = {
            ...data,
            functions: data.functions.filter(fn => fn.toLowerCase().includes(query)),
            types: data.types.filter(type => type.toLowerCase().includes(query)),
            constants: data.constants.filter(constant => constant.toLowerCase().includes(query)),
            submodules: data.submodules.filter(submod => submod.name.toLowerCase().includes(query))
          };
  ...
  async getBatchModuleCounts(moduleNames: string[]): Promise<Map<string, ModuleCounts>> {
