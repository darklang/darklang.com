...
const CodeSample: React.FC<CodeSampleProps> = ({ content, className = "" }) => {
  ...
  const formatContent = (code: string) => {
    return code
      .replace(
        /(Made less than a minute ago)/g,
        '<span class="text-gray-500">$1</span>',
      )
      .replace(
        /(request:|body:|headers:|info:|fullBody:|accept:|content-length:)/g,
  ...
  return (
    <div
      className={`bg-blue-50 p-4 rounded-md text-xs font-mono overflow-x-auto leading-relaxed ${className}`}
    >
      <pre
