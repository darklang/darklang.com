...
let listOpen () : Http.Response =
  let tickets = Stdlib.DB.query Tickets (fun t -> t.isOpen)
  let body = Stdlib.Json.serialize<List<Ticket>> tickets
  Stdlib.Http.responseWithJson body 200

let router (req: Http.Request) : Http.Response =
...
rows
|> Stdlib.List.iter (fun p -> Stdlib.DB.set p p.sku Products)

Builtin.printLine $"imported {Stdlib.List.length rows} products"`;

const Cmd: React.FC<{ cmd: string; out: string }> = ({ cmd, out }) => (
  <div className="rounded-xl border border-[#333336] bg-dark-black px-4 py-3 font-code text-[13px] leading-relaxed overflow-x-auto whitespace-pre">
  ...
  <section className="py-16 md:py-20">
    <div className="max-w-7xl 2xl:max-w-[100rem] mx-auto px-4">
      <div className="max-w-3xl mb-4">
        ...
        >
          A handler, a database and a command-line tool are all{" "}
          <span className="text-purple-lbg">functions and values</span> in the
          same tree
        </SectionTitle>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {CARDS.map(c => (
          ...
          >
            ...
            <div className="rounded-xl border border-gray-200 bg-[#F9F9FB] p-4 overflow-x-auto">
              <CodeDisplay code={c.code} showLineNumbers={false} size="sm" />
            </div>
